...
class LongSubmergedGeneratorTests(unittest.TestCase):
    def test_generator_keeps_ventilation_vanilla_and_patches_air_capacity(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertEqual(manifest["assemblyName"], "LongSubmerged10xPatch_1_4_3")
            ...
            self.assertIn('private const string RuntimeVersion = "1.4.3";', runtime_patch_text)
            ...
            self.assertIn("FlamesRegistrySpawnParticlesPatch", runtime_patch_text)
            self.assertIn('HarmonyPatch(typeof(FlamesRegistry), "SpawnParticles", new Type[] { })', runtime_patch_text)
            self.assertIn("ShouldRunFlamesRegistrySpawnParticles", runtime_patch_text)
            self.assertIn("HandleFlamesRegistrySpawnParticlesException", runtime_patch_text)
            self.assertIn("FlamesRegistry particle guard prevented crash", runtime_patch_text)
            self.assertIn("flamesEffect2", runtime_patch_text)
            self.assertIn("NullReferenceException absorbe", runtime_patch_text)
