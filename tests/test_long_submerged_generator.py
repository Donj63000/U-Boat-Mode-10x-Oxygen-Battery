...
class LongSubmergedGeneratorTests(unittest.TestCase):
    ...
    def test_generator_keeps_surface_oxygen_vanilla_and_runtime_safe(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            dropdown_template_text = extract_csharp_block(runtime_patch_text, "private RectTransform CreateDropdownTemplate")
            self.assertIn("templateRect.anchorMin = new Vector2(0f, 1f);", dropdown_template_text)
            self.assertIn("templateRect.anchorMax = new Vector2(1f, 1f);", dropdown_template_text)
            self.assertIn("templateRect.pivot = new Vector2(0.5f, 0f);", dropdown_template_text)
            self.assertIn("templateRect.anchoredPosition = new Vector2(0f, 32f);", dropdown_template_text)
            self.assertNotIn("templateRect.anchoredPosition = new Vector2(0f, -32f);", dropdown_template_text)
            self.assertIn("Canvas templateCanvas = template.AddComponent<Canvas>();", dropdown_template_text)
            self.assertIn("templateCanvas.overrideSorting = true;", dropdown_template_text)
            self.assertIn("templateCanvas.sortingOrder = CanvasSortingOrder + 2;", dropdown_template_text)
            self.assertIn("template.AddComponent<GraphicRaycaster>();", dropdown_template_text)
