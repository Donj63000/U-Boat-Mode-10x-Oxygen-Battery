...
class LongSubmergedGeneratorTests(unittest.TestCase):
    def test_generator_keeps_surface_oxygen_vanilla_and_runtime_safe(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertEqual(manifest["assemblyName"], "LongSubmerged10xPatch_1_4_7")
            ...
            self.assertIn('private const string RuntimeVersion = "1.4.7";', runtime_patch_text)
            ...
            self.assertIn("RuntimeSettingsVersion = 10", runtime_patch_text)
            ...
            self.assertIn("Je ne patche plus Resource.UpdateAmount en 1.4.7", runtime_patch_text)
            self.assertNotIn("UpdateBatteryResourceAfterVanilla", runtime_patch_text)
            self.assertNotIn("ResourceUpdateAmountBatteryPatch", runtime_patch_text)
            ...
            self.assertNotIn('HarmonyPatch(typeof(Resource), "UpdateAmount")', runtime_patch_text)
            ...
            self.assertNotIn('HarmonyPatch(typeof(AirCompressor), "OnEnable")', runtime_patch_text)
            self.assertNotIn('HarmonyPatch(typeof(AirCompressor), "EnergyUsage_Changed")', runtime_patch_text)
            self.assertNotIn('HarmonyPatch(typeof(Ventilation), "OnEnable")', runtime_patch_text)
            ...
            self.assertNotIn('reason + ".AirCompressor"', runtime_patch_text)
            self.assertNotIn('reason + ".Ventilation"', runtime_patch_text)
            ...
            self.assertIn("private const float OxygenRuntimeMaxFactor = 1800f;", runtime_patch_text)
            ...
            self.assertIn("Mega torpilles : runtime F10 reglable 1-100, degats defaut x10, effets visuels bornes x3", report_text)
