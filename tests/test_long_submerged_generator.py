...
class LongSubmergedGeneratorTests(unittest.TestCase):
    def test_generator_keeps_ventilation_vanilla_and_patches_air_capacity(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertEqual(manifest["assemblyName"], "LongSubmerged10xPatch_1_4_1")
            ...
            self.assertIn('private const string RuntimeVersion = "1.4.1";', runtime_patch_text)
            self.assertIn("Runtime patch loaded v", runtime_patch_text)
            self.assertIn("PatchAll()", runtime_patch_text)
            self.assertIn("Harmony PatchAll a echoue", runtime_patch_text)
