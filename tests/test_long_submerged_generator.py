...
class LongSubmergedGeneratorTests(unittest.TestCase):
    def test_github_presentation_matches_generator_defaults(self) -> None:
        ...
        self.assertIn("| **Interior Lighting** | `Lumières orange/vert = on` |", readme_text)
    ...
    def test_generator_keeps_surface_oxygen_vanilla_and_runtime_safe(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertIn("Lumieres orange/vert F10 activees par defaut avec restauration vanilla quand decochees", manifest["description"])
            ...
            self.assertIn('SetColorProperty(controller, "AlarmColor", controller.AlarmColor, AlarmOrangeColor);', runtime_patch_text)
            self.assertIn('SetColorProperty(controller, "BlueColor", controller.BlueColor, SilentRunGreenColor);', runtime_patch_text)
            self.assertIn('SetColorProperty(fillLight, "RedColor", fillLight.RedColor, AlarmOrangeColor);', runtime_patch_text)
            self.assertIn('SetColorProperty(fillLight, "BlueColor", fillLight.BlueColor, SilentRunGreenColor);', runtime_patch_text)
            ...
            self.assertIn("RuntimeSettingsVersion = 18", runtime_patch_text)
            ...
            self.assertIn('CreateToggle(panelObject.transform, "Lumières orange/vert"', runtime_patch_text)
            ...
            self.assertIn("Eclairage interieur : case F10 Lumieres orange/vert activee par defaut ; decochee restaure les couleurs vanilla", report_text)
            ...
            self.assertIn("Toggle Lumières orange/vert : actif par defaut, decoche = couleurs vanilla", generated_readme_text)
            self.assertIn("Lumières orange/vert : case F10 active par defaut, decochee pour restaurer les couleurs vanilla", generated_readme_text)
            self.assertIn("Eclairage interieur : rouge Alarm remplace visuellement par orange ambre, bleu SilentRun remplace visuellement par vert, gameplay inchange quand la case est activee", generated_readme_text)
            self.assertIn("La lumiere SilentRun est verte uniquement au rendu quand Lumières orange/vert est cochee", generated_readme_text)
