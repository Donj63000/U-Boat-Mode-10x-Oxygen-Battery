...
class LongSubmergedGeneratorTests(unittest.TestCase):
    def test_github_presentation_matches_generator_defaults(self) -> None:
        ...
        self.assertIn("- Heavy Armor migration: v1.4.13 turns old saved Heavy Armor settings off once, then keeps your F10 choices.", install_text)
        ...
        self.assertIn("| **Heavy Armor** | `Heavy Armor = off` |", readme_text)
        self.assertIn("| **Heavy Armor** | `off` | `off` | `on`, x3 protection |", readme_text)
    ...
    def test_generator_keeps_surface_oxygen_vanilla_and_runtime_safe(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertIn("OrangeAlarmLightingPatcher", runtime_patch_text)
            self.assertIn("internal static class OrangeAlarmLightingPatcher", runtime_patch_text)
            self.assertIn("private static readonly Color AlarmOrangeColor = new Color(1f, 0.55f, 0.12f, 1f);", runtime_patch_text)
            self.assertIn("typeof(OrangeAlarmPlayerShipInteriorLightingAwakePatch)", runtime_patch_text)
            self.assertIn("typeof(OrangeAlarmPlayerShipInteriorLightingStartPatch)", runtime_patch_text)
            self.assertIn("typeof(OrangeAlarmPlayerShipInteriorLightingApplyPresetsPatch)", runtime_patch_text)
            self.assertIn("typeof(OrangeAlarmLightControllerUpdatePresetsPatch)", runtime_patch_text)
            self.assertIn("typeof(OrangeAlarmFillLightUpdatePresetsPatch)", runtime_patch_text)
            ...
            self.assertIn('CreateToggle(panelObject.transform, "Blindage lourd"', runtime_patch_text)
            ...
            self.assertIn('HarmonyPatch(typeof(Hull), "AddDamage")', runtime_patch_text)
            self.assertIn('HarmonyPatch(typeof(Equipment), "AddDamage")', runtime_patch_text)
            self.assertIn('HarmonyPatch(typeof(Equipment), "AddWaterDamage")', runtime_patch_text)
            self.assertIn('HarmonyPatch(typeof(PlayableCharacter), "AddDamage")', runtime_patch_text)
            self.assertIn('HarmonyPatch(typeof(DamageUtility), "DoApplyDamage")', runtime_patch_text)
            ...
            self.assertIn("ScalePlayerCrewAndEffects", runtime_patch_text)
            ...
            self.assertIn("Finalizer(bool __state)", runtime_patch_text)
            ...
            self.assertIn("if (!IsHeavyArmorActive() || !CouldAffectPlayerShip(target, position, damageRadius))", runtime_patch_text)
            ...
            self.assertNotIn("using UBOAT.Game.Scene.Effects;", runtime_patch_text)
