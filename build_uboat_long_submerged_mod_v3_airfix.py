...
Commande conseillée :
    py build_uboat_long_submerged_mod_v3.py --uboat "C:\\Program Files (x86)\\Steam\\steamapps\\common\\UBOAT" --force --clear-cache
...
# La v2 modifiait la ligne ventilation deux fois sur EnergyUsage et montait RegenerationLimit trop haut.
...
def patch_parameter_cell(
    ...
    # Batterie : on garde le comportement de la v2, parce que tu as validé que la batterie fonctionne.
...
def write_manifest(mod_dir: Path, args: argparse.Namespace) -> None:
    manifest = {
        ...
        "description": (
            ...
            "v3 garde la ventilation vanilla par défaut."
        ),
        "author": MOD_AUTHOR,
...
def parse_args(argv: list[str]) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Génère le mod UBOAT Long Submerged 10x+ v3.",
    )
...
def main(argv: list[str]) -> int:
    ...
    print("\n=== Long Submerged 10x+ v3 généré avec succès ===")
    ...
    print("  7. Si la ventilation était cassée par la v2, cette v3 doit la remettre vanilla car elle ne copie plus la ligne Ventilation.")
